...
@Singleton
internal class DefaultTorrentsApi(private val requester: HttpRequester) : TorrentsApi {
    override suspend fun getTorrents(
        page: Int,
        limit: Int
    ): Either<AppError, PaginatedResponse<TorrentDto>> = requester.request {
        get("anime/torrents") {
            parameter("page", page)
    ...
    override suspend fun getTorrentFile(
        hashOrId: String,
        pk: String?
    ): Either<AppError, ByteArray> = requester.request {
